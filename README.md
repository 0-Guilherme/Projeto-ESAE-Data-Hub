...
3.  **Tabela de Usuários (Moodle)**
    > Apresenta os mesmos dados da lista de gestão, mas em um formato de tabela densa, utilizando o controlo Data Table. Esta vista é otimizada para a análise de um grande volume de dados no ecrã e serve de base para futuras funcionalidades de exportação.
...
5.  **Gestão de Cursos**
    > Ecrã de gestão do catálogo de cursos (`LST_Cursos`), com uma interface e lógica semelhantes ao ecrã de gestão de utilizadores. Permite a busca por nome ou ID do curso e a navegação para os detalhes de um curso específico.
...
8.  **Histórico de Conclusões Recentes**
    > Tela de relatório dinâmico que exibe todas as conclusões de cursos que ocorreram num período de tempo definido. Inclui controlos de filtro interativos, como seletores de data e botões para períodos pré-definidos ("Esta Semana", "Semana Passada").
    ![Tela 8](Documentacao/Imagens/8.jpg)

9.  **Dashboard Interativo de Cursos (BI)**
    > Ecrã de análise visual focado nos cursos. Apresenta um gráfico interativo (construído com uma galeria) que mostra os cursos mais populares com base no número de alunos. Inclui menus suspensos que permitem ao gestor filtrar o gráfico por Categoria e por Período (Semestre/Ano).
    ![Tela 9](Documentacao/Imagens/9.jpg)

10. **Gestão de Usuários SAE**
    > Ecrã principal para a gestão da lista de referência `LST_Usuarios_SAE`. Inclui uma galeria com filtros avançados que permitem ao administrador visualizar os utilizadores por `Status` ("Ativo", "Inativo", "Verificar!") e por `OrigemDados`.
    ![Tela 10](Documentacao/Imagens/10.jpg)

11. **Tela de Auditoria de Inconsistências**
    > Ecrã de "ações necessárias" que exibe uma lista filtrada apenas com os utilizadores que o fluxo "auditor" do Power Automate sinalizou como tendo dados inconsistentes entre as listas `LST_Usuarios` e `LST_Usuarios_SAE`.
...
13. **Tela de Detalhes e Reconciliação (SAE vs. Moodle)**
    > Tela de análise comparativa. Apresenta dois formulários lado a lado, mostrando as informações de um mesmo utilizador como estão na lista `LST_Usuarios_SAE` e como estão na lista principal `LST_Usuarios` (Moodle), facilitando a identificação de inconsistências.
    ![Tela 13](Documentacao/Imagens/13.jpg)

14. **Análise do Alerta (Inconsistência Tipo 1)**
    > Detalhe do fluxo de auditoria. Esta imagem exemplifica um utilizador que foi sinalizado para correção. A mensagem de alerta indica a natureza da inconsistência (ex: "Status no Moodle é 'Não', mas está 'Ativo' na lista SAE").
    ![Tela 14](Documentacao/Imagens/14.jpg)

15. **Análise do Alerta (Inconsistência Tipo 2)**
    > Segundo exemplo do fluxo de auditoria. A mensagem de alerta em amarelo destaca uma outra regra de negócio (ex: "Status no Moodle é 'Sim', mas está 'Inativo' na lista SAE").
...
17. **Dashboard de BI (Power BI) - Tela Inicial**
    > Tela principal do relatório no Power BI, que é integrado ao Power App. Apresenta os KPIs (Indicadores-Chave de Performance) e os principais filtros que se aplicam a todo o relatório.
    ![Tela 17](Documentacao/Imagens/17.jpg)

18. **Dashboard de BI - Análise Comparativa**
    > Vista do relatório no Power BI que permite o estudo de campo amostral, cruzando dados administrativos (como Lotação e Cargo do Teams) com os dados de participação em cursos vindos do Moodle.
    ![Tela 18](Documentacao/Imagens/18.jpg)

19. **Dashboard de BI - Distribuição por Lotação**
    > Gráfico de barras ou pizza no Power BI que exibe a distribuição quantitativa de participantes por lotação, permitindo identificar os departamentos com maior ou menor engajamento.
    ![Tela 19](Documentacao/Imagens/19.jpg)

20. **Dashboard de BI - Distribuição por Cargo**
    > Análise visual no Power BI que mostra a distribuição de participantes por cargo, ajudando a entender quais perfis profissionais mais participam das capacitações.
    ![Tela 20](Documentacao/Imagens/20.jpg)

21. **Dashboard de BI - Análise Temporal**
    > Gráfico de linhas ou colunas no Power BI que exibe a evolução do número de inscrições e conclusões ao longo do tempo (por mês ou por semestre), permitindo a análise de tendências.
